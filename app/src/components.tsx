...
const SHOWS_SAVINGS = new Set<RecAction>(["retirement-candidate", "retirement-candidate-blocked", "merge"]);
      ...
      {excludedDupes.length > 0 && (
        <details open>
          ...
          <p className="muted" style={{ margin: "6px 0 8px" }}>
            ...
            dataset). Surfaced for transparency, not consolidation — retiring a system model breaks the feature that
            owns it.
          </p>
          <table className="grid">
            <thead>
...
export function Insights({ clusters, chains }: { clusters: Cluster[]; chains: PromotionChain[] }) {
  const retirable = clusters.reduce((n, c) => n + (c.members.length - 1), 0);
  const driftChains = chains.filter((c) => c.drift).length;
  if (retirable === 0 && driftChains === 0)
    return (
      <div className="insights">
